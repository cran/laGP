...
%\VignetteDepends{laGP,MASS,lhs,akima,tgp}
%\VignettePackage{laGP}

\author{Robert B. Gramacy\\The University of Chicago\\Booth School of Business}
...
\Abstract{
  ...
  severely limits the size of data---training and testing---on which they can
  be deployed.  That means that in computer experiment, spatial/geo-physical,
  and machine learning contexts, GPs no longer enjoy privledged status as
  modern data sets continue balloon in size. We discuss an implementation of
  local approximate Gaussian process models, in the \pkg{laGP} package for
  \proglang{R}, which offers a particular sparse-matrix remedy uniquely
  ...
  symmetric multi-processor and graphical processing unit support which are
  not enabled by default, and the built-in wrapper routines that automate
...
\Address{
  ...
  E-mail: \email{rbgramacy@chicagobooth.edu}\\
  URL: \url{http://faculty.chicagobooth.edu/robert.gramacy/}
}
...
\section{Gaussian process regression and sparse approximation}

Gaussian process (GP) regression models, sometimes called a Gaussian spatial
process (GaSP), have been popular for decades in spatial data contexts like
geostatistics \citep[e.g.,][]{cressie:1993} where they are known as {\em
kriging} \citep{math:1963}, and in computer experiments where they are
...
enjoyed by other general-purpose approaches to nonpatrametric nonlinear
modeling, and because they perform well in out-of-sample tests.  They are not, however,
without their drawbacks, and two important ones are computational tractability and
...
(X_N, Y_N)$, comprised of $N \times p$-dimensional design $X_N$ define a
multivariate normal (MVN) likelihood for an
$N$-vector of scalar responses $Y_N$ through a small number of parameters $\theta$
that describe how $X_N$ is related to $\mu_N$ and $\Sigma_N$.  Linear regression
is a special case where $\theta = (\beta, \tau^2)$ and $\mu_N = X_N \beta$
and $\Sigma_N = \tau^2 I_N$.
...
\citep[e.g.,][]{sant:will:notz:2003}, which is the simplifying assumption we shall
make throughout, although it is easy to generalize to a mean described by a 
polynomial basis.  Let $K_\theta(x, x')$ be a correlation function so
...
$X_N$.  Here we are changing the notation slightly so that $\theta$ is reserved
explicity for $K_\theta$, isolating $\tau^2$ as a separate scale parameter.
Choices of $K_\theta(\cdot, \cdot)$ determine stationarity,
...
more robust to modeling computer experiments
...
all parameters. Although the \pkg{laGP} package favors the
isotropic Gaussian case, we argue that many of the drawbacks of that overly
simplistic choice, leading theorists and practitioners alike to prefer other
choices like the Mat\'ern \citep{stein:1999}, are less of a concern in our
particular approach to sparse approximation.  The package also provides
a limited set of routines which can accommodate a separable Gaussian correlation
...
\citep[e.g.,][Chapter 5]{rasmu:will:2006}.\footnote{Eq.~(\ref{eq:gpk})
...
Z <- sin(X)
@
The code below uses some low-level routines in the package to initialze
a GP representation with $\theta=2$ and $\eta=10^{-6}$.  Then, a derivative-based
MLE sub-routine is used to find $\hat{\theta}_{N=7}$, maximizing (\ref{eq:gpk}).
<<>>=
...
@
Finally, Figure \ref{f:sin} provides a visualization of those sample paths
...
$Y(x)|D_N, \hat{\theta}_N$. Obseve how the predictive variance narrows for $x$
...
$O(N^3)$ matrix decompositions required to calculate $K_N^{-1}$ and $|K_N|$
in Eqs.~(\ref{eq:gpk}--\ref{eq:preds2}).  In practice that limits $N$ to the
low thousands for point inference, and high hundreds for sampling-based
inference like the bootstrap or Bayesian MCMC.  This has pushed some practitioners
towards wholly new modeling apparatuses, say via trees 
\citep{pratola:etal:2013,gramacy:taddy:wild:2013,chipman:ranjan:wang:2012}.  Although
...
extending the $N$ which could be accommodated by an order of magnitude.
\citet{paciorek:etal:2013} developed a package for \proglang{R} called
\pkg{bigGP} that combined symmetric-multiprocessor, cluster, and GPU facilities
to gain yet another order of magnitude.  \citeauthor{paciorek:etal:2013} were
able to handle $N=67275$. To go too far down that road, however, may miss the
point in certain contexts.  Computer model emulation is meant to {\em avoid}
expensive computer simulation, not be a primary consumer of it.
...
composite likelihood approach, rather than directly leveraging spare matrix
...
\citet{pratola:etal:2013} found it necessary to modify a tree-based approach
for distribution over the nodes of a supercomputer to handle an $N=7$M sized
design.
...
order to order to handle large-$N$ problems in reasonable computation times
...
Sections \ref{sec:examples} and \ref{sec:calib} devoted to larger scale and
more exhaustive demonstration of the features in action, first demonstrating
local emulation and regression/smoothing and then adapting them to the
important problem of large scale computer model calibration.  Section
\ref{sec:hooks} discusses extra features of the package which are useful
in other contexts, like the optimization of blackbox functions, and the 
potential for customization and extension of existing subroutines in the package.
The appendix describes how the package can be compiled to enable SMP and
GPU support, and discusses a variation a the key wrapper function \code{aGP}, which
enables distribution of predictions across the nodes of a cluster.
...
data GP regression.  They utilize a sparse representation, but in fact only
work with small dense matrices.  And the many-independent nature of the
calculations required are amenable to massive parallelization.  The result is
an approximate GP regression capability that can accommodate orders of
magnitude larger training and testing sets than ever before.  The method can
be seen as a modernization of {\em local kriging} from the spatial statistics
literature \citep[][pp,131--134]{cressie:1993}.  It involves focusing on
approximate predictive equations at a particular generic location, $x$,
via a subset of the data $D_n(x) \subseteq D_N$, where the sub-design $X_n(x)$
is (primarily) comprised of $X_N$ close to $x$.  The thinking is that, with
the typical choices of $K_\theta(x, x')$, where correlation between elements
$x' \in X_N$ decays quickly for $x'$ far from $x$, remote $x'$s have
vanishingly small influence on prediction anyways.  Ignoring them in order
to work with much smaller, $n$-sized, matrices will bring a big computational
savings with little impact on accuracy.

This is a sensible idea:  It can be shown to induce a valid stochastic
process; when $n \ll 1000$ the method is fast and accurate, and as $n$ grows
the predictions increasingly resemble their full $N$-data counterparts; and
for smaller $n$ $V_n(x)$ is organically inflated relative to $V_N(x)$,
acknowledging greater uncertainty in approximation.  The simplest version of
such a scheme would be via nearest neighbors (NN):  $X_n(x)$ comprised of closest
elements of $X_N$ to $x$.  \citet{emory:2009} showed that this works well for
many common choices of $K_\theta$.  However, NN designs are known to be
sub-optimal \citep{vecchia:1988,stein:chi:welty:2004} as it pays to have some
spread in $X_n(x)$ in order to obtain good estimates of correlation
hyperparameters like $\theta$.  Still, searching for the optimal sub-design,
which involves choosing $n$ from $N$ things, is a combinatorially huge
undertaking.

\citet{gramacy:apley:2014} showed how a greedy search could provide
...
elaborate below, along with the the other relevant GP quantities for
inference and prediction (\ref{eq:gpk}--\ref{eq:preds2}) can be updated as $j
\rightarrow j+1$ in $O(j^2)$ time as long as the parameters, $\theta$, remain
constant across iterations.  Therefore over the entirety of those iterations,
$j=n_0, \dots, n$, the scheme is in $O(n^3)$.  The idea of sequential updating
for GP inference, whether for design or for speed, is not new
\citep{gramacy:polson:2011,haaland:qian:2012}, however the focus of previous
approaches has been global.  Working local to particular $x$
brings both computational and modeling/accuracy advantages.

\subsection{Criterion for local design}

\citeauthor{gramacy:apley:2014} considered two criteria in addition to NN, one being
...
equation \eqref{eq:predgp}, except that the $j\!+\!1$ subscript has been added
in order to indicate dependence on $x_{j+1}$ and the future, unknown
$y_{j+1}$. They then derive the approximation
\begin{equation}
...
\end{equation}
%
The first term in (\ref{eq:mspe}) estimates predictive variance at $x$ after $x_{j+1}$ is
...
(\ref{eq:mspe}) estimates the rate of change of the predictive mean at $x$,
...
full MSPE criteria tends to yield qualitatively similar local designs
$X_n(x)$ as ones obtained using just $V_j(x|x_{j+1}; \hat{\theta}_j)$, but at a
...
\begin{align}
 v_j(x&; \theta)  - v_{j+1}(x; \theta) \label{eq:dxy} \\
&= k_j^\top(x) G_j(x_{j+1}) m_j^{-1}(x_{j+1}) k_j(x) + 2k_j^\top(x)
  g_j(x_{j+1}) K(x_{j+1},x) + K(x_{j+1},x)^2 m_j(x_{j+1}), \nonumber
\end{align}
where $G_j(x') \equiv g_j(x') g_j^\top(x')$,
\begin{equation}
 g_j(x') = -m_j(x') K_j^{-1}
k_j(x') \;\;\;\;\; \mbox{and} \;\;\;\;\; m_j^{-1}(x') = K_j(x',x') -
k_j^\top(x') K_j^{-1} k_j(x'). \label{eq:piedef}
\end{equation}
Observe that only $O(j^2)$ caluclations are required above. Although known for
 some time in other contexts,
\citeauthor{gramacy:apley:2014} chose the acronym ALC to denote the use
of that decomposition in local design in order to recognize a similar approach
...
consider the following gridded design in $[-2,2]^2$.
<<cache=TRUE>>=
x <- seq(-2, 2, by=0.02)
...
responses---a point which we will discuss at greater length shortly.  The synthetic
response \citeauthor{gramacy:apley:2014} used for illustrations is coded below, and
we shall elucidate that nature of input/output relationships
in due course.
<<cache=TRUE>>=
f2d <- function(x)
  {
...
@
Both designs use $n_0=6$ nearest neigbors to start, make greedy selections until
...
\citet{gramacy:haaland:2014} offer an explanation, which surprisingly has
little to do with the particular choice of $K_\theta$. Consider two potential
locations, an $x_{j+1}$ close to $x$  and $x_{j+1}'$ farther away. One
naturally wonders: how could the latter choice, $x_{j+1}'$ with
$y_{j+1}'$-value exponentially less correlated with $y(x)$ than $y_{j+1}$ via
$x_{j+1}$, be preferred over the closer $x_{j+1}$ choice? The answer lies the
form of (\ref{eq:piedef}). Although quadratic in $K_\theta(x_{j+1}, x)$, the
``distance'' between the $x$ and the potential new local design location
$x_{j+1}$, it is also quadratic in  $g_j(x_{j+1})$, a vector measuring
``inverse distance'', via $K_j^{-1}$, between $x_{j+1}$ and the current local
design $X_j(x)$. So the criteria makes a tradeoff: minimize ``distance'' to
$x$ while maximizing ``distance'' (or minimizing ``inverse distance'') to the
existing design.   Or in other words, the potential value of new design
element $(x_{j+1}, y_{j+1})$ depends not just on its proximity to $x$, but
also on how potentially different that information is to where we already have
(lots of) it, at $X_j(x)$.
...
output at the end are derived based on a local MLE calculation given the data $D_n(x)$.
<<>>=
p.mspe$mle
p.alc$mle
@
...
More about local inference for $\theta$ is deferred until Section \ref{sec:global}.
For now we note that the implementation is same as the one behind the \code{mleGP}
routine described earlier in Section \ref{sec:gp}.
...
since it doesn't require evaluation of derivatives, etc.  Although a more
...
together with the simpler implementation, led \citeauthor{gramacy:apley:2014} to
...
computation for each $x$. However, independence allows trivial
parallelization requiring token programmer effort.  When compiled
correctly [see Appendix \ref{sec:omp}] the  \pkg{laGP} package can 
exploit symmetric multiprocessor (SMP) via \code{OpenMP} pragmas in its
underlying \proglang{C} implementation.  The simplest way this is
accomplished is via a ``parallel-for'' pragma.
\begin{verbatim}
#ifdef _OPENMP
...
processors scale roughly as $1/P$.  Later versions of the package use
the ``\code{parallel}'' pragma which has slightly less overhead. 
...
YY <- f2d(XX)
@
Now, we use the \code{aGP} function to iterate over the elements of $\tilde{X}
\equiv$\ \code{XX}.  The package used in this illustration is compiled for
\code{OpenMP} support, and executed on a 4-core hyperthreaded 2010 iMac.  Therefore
any timings are 7-8x faster than on a similar one-core run. 
<<cache=TRUE>>=
nth <- 8
P.alc <- aGP(X, Y, XX, omp.threads=nth, verb=0)
@
Note that the default method is ALC.  The results obtained with \code{method = "mspe"}
are similar, but require more computation time.  Further comparison is delayed until
Section \ref{sec:examples}.  The \code{verb=0} argument suppresses a progress meter 
...
surface at $x_w = 0.51$ along with the true responses (completely covered by
...
enforced---calculations at nearby locations are independent and occur
potentially in parallel---the resulting surface looks smooth to the eye.
...
@
\caption{Bias in the predictive mean surface shown in Figure \ref{f:aagp-s}.}
\label{f:aagp-se}
\end{figure}
...
stationary Gaussian process predictor.  Here, stationarity loosely means that
the covariance structure is modeled  uniformly across the input space. Most
choices of $K_\theta(\cdot, \cdot)$, like the isotropic Gaussian we use,
...
degree of non-stationarity.  In fact, by default, \code{aGP} goes beyond
that by learning separate $\hat{\theta}_n(x)$ local to each $x \in \mathcal{X}$
by maximizing the local likelihood (or posterior probabilities). 
\begin{figure}[ht!]
\centering
...
@
\caption{Spatially varying lengthscale estimated along the slice shown in Figure \ref{f:aagp-s}.}
\label{f:aagp-sd}
\end{figure} 
Figure \ref{f:aagp-sd} shows indeed that the estimated lengthscales vary spatially.  However,
apparently even this extra degree of flexibility is not enough to mitigate the
small amount of bias shown in Figure \ref{f:aagp-se}.

\citeauthor{gramacy:apley:2014} recommend a two-stage scheme wherein the above
process is repeated wherein new $X_n(x)$ are chosen conditional upon
...
globally non-stationary model and generally more accurate predictions than the
single-stage scheme.
\begin{figure}
\centering
\fbox{
...
variance scheme, via Eq.~(\ref{eq:dxy}), although MSPE (\ref{eq:mspe}) or
any other criteria could be deployed there, at each greedy stage of local
design. Of course, more than two repetitions of the global search scheme can
be performed, but in many examples two has been sufficient to achieve rough
convergence of the overall iterative scheme.  Optionally, the
$\hat{\theta}_n(x)$-values can be smoothed (e.g., by \pkg{loess}) before they
are fed back into the local design schemes.  Considering other popular
approaches to adapting a stationary model to achieve nonstationary surfaces,
which usually involve orders of magnitude more computation \citep[e.g.][and
references therein]{schmidt:ohagan:2003}, this small adaptation is a thrifty
alternative that does not change the overall computational order of the
scheme.
...
run, stored in the \code{P.alc} object.  Compariing the predictions
...
wide range of examples.  For example \citet{gramacy:apley:2014} provided an
...
non-stationarity, second stage local designs do not solve the bias problem
completely.  The method is still locally stationary, and indeed locally
isotropic in its \pkg{laGP} implementation.  Finally, the subsequent stages
of design tend to be slightly faster than earlier stages since the number
of Newton iterations required to converge on $\hat{\theta}_n(x)$ is
reduced given refined starting values for search.
% <<>>=
% c(P.alc$time, P.alc2$time)
% @
...
(\ref{eq:preds2}) in some way, every candidate entertained involves an
...
several reasons to remain cheery about computational aspects.  One is that
$O(Nn^3)$ is not $O(N^3)$.  The others require more explanation, and
potentially slight adjustments in implementation.

Not all $N-j$ need be entertained for the method to work well.  For the same
reason prediction is localized to $x$ in the first place, that correlation
decays quickly away from $x$, we can probably afford to limit search to $N'
\ll N-j$ candidates near to $x$.  By default, \code{laGP} and \code{aGP} limit
search to the nearest $N' = 1000$ locations, though this can be adjusted
with the \code{close} argument.  One can check [not shown here] that increasing
...
perfect for graphical processing unit (GPU) parallelization.  Each potential
candidate, up to 65K candidates, can be entertained on a seprate GPU block,
and threads within that block can be used to perform many of the required
dense linear algebra operations in Eq.~(\ref{eq:dxy}) in parallel.  In
...
Finally, \citet{gramacy:haaland:2014} suggested that the discrete and exhaustive
...
via Eq.~(\ref{eq:dxy}))---and observed many that regularities persist over choices
of $K_\theta(\cdot, \cdot)$ and its parameterization.  As long as the $X_N$ is
...
The code below calculates an such an ALC-ray based design, augmenting our
...
not idential to the others but clearly exhibits similar qualitative features.
...
p
@
\citeauthor{gramacy:haaland:2014} recommend using $p$ rays per greedy search
...
@
The result is a global predictor which is 
\Sexpr{signif((P.alc$time + P.alc2$time)/(P.alcray$time + P.alcray2$time), 2)} times
faster, echoing the single-$x$ results form \code{laGP} above
<<>>=
...
rmse
@
Time in \Sexpr{signif(P.alcray$time + P.alcray2$time, 2)} seconds on a 2010
...
present three further examples which offer a more convincing demonstration of
...
noisy data can be accommodated by estimating local nuggets.  The following
section provides a further example of how it can be deployed for computer
model calibration.
...
\citet{gramacy:apley:2014} without requiring too many compute cycles.
<<cache=TRUE>>=
N <- 100000
Npred <- 1000
dim <- 8
library(lhs)
@
The experiment involves ten repetitions of inputs from a random Latin hypercube sample
\citep[LHS;][]{mckay:conover:beckman:1979} which generate the training data and testing
sets, with responses from \code{borehole}, and automates calculations of a sequence of
(local GP) estimators fit to the training sets followed by out-of-sample RMSE
calculations on the testing sets.  Storage for those RMSEs, along with timing info, is allocated as follows
<<cache=TRUE>>=
T <- 10
nas <- rep(NA, T)
...
The \code{for} loop below iterates over each Monte Carlo repetition.
<<cache=TRUE>>=
for(t in 1:T) {
...
\citeauthor{gramacy:apley:2014}'s similar experiments included variations
on the method of compactly supported covariances (CSC) \citep{kaufman:etal:2012}
which provided estimators with similar accuracies, but required at least
an order magnitude more compute time.  In fact, they commented that $N=10000$
was the limit that CSC could accommodate on their machine due to memory swapping
issues.
...
plain NN-based emulation strategy (with local inference for $\hat{\theta}_n(x)$;
i.e., \code{nn} in the table), a greedy design is three times more accurate
out-of-sample.  \citet{gramacy:haaland:2014} show that the trend continues as
$N$ is increased, indicating the potential for extremely accurate emulation on
testing and training sets of size $N > 1$M in a few hours.  Pairing with
cluster-style distribution, across 96 16-CPU nodes, that can be reduced to 188
seconds, or extended to $N > 8M$ in just over an hour.  They show that smaller
(yet still large) designs $N < 100000$, searching exhaustively rather than by
rays leads to more accurate predictors.  In those cases, massive
parallelization over a cluster and/or with GPUs
\citep{gramacy:niemi:weiss:2014} can provide accurate predictions on a
...
Newton-like methods are vastly simplified.  When deployed for local inference
...
is not limited to this choice.  Indeed \citet{gramacy:apley:2014} developed
...
function in the context of {\em global}, that is canonical, GP inference.
...
the code below considers ten random subsets of size $n=1$K from the
...
together with analytic derivatives of the log likelhood; the function
...
derivatives. The package also offers \code{jmleGPsep}, an analog of
\code{jmleGP}, which automates a profile-like approach to iterating over
$\theta | \eta$ and $\eta | \theta$ where the latter is performed with a
Newton-like scheme leveraging first and second derivatives.  We do not
...
size (8-dim) inputs, from the borehole experiment, leads to estimating
near-zero nuggets and ill-conditioning in the matrix decompositions, arising
from our choice of Gaussian correlation function.

For estimating nuggets in this setup, where the response is both deterministic
and extremely smooth (and stationary), we recommend the \pkg{GPfit} \citep{gpfit}
based on the methods of \citet{ranjan:haynes:karsten:2011}.  However, we caution
that our experience is \pkg{GPfit} can be slow on data sets as large as $N=1000$.
...
distribution that is highly concentrated near small values suggesting that it
...
from treating some inputs differently than others.
<<>>=
...
@
is competitive with the best methods in the study above---which are based on
$n=200$ whereas only the default $n=50$ was used here.  Also observe that the
RMSE we just obtained is better than half of the one we reported
...
off-line, if so inclined, and permit us and simply conclude here that it can
be beneficial to perform a cursory global analysis with a separable
correlation function to determine if the inputs should be scaled before
performing a local (isotropic) analysis on the full data set.
...
in the first moments after an impact.  It can be found
 in the \pkg{MASS}
package for \proglang{R}.  For comparison, we first fit a simple GP
...
smoothness ($n=132$, reproducing the full GP results exactly) and adaptivitity
...
\label{f:mcycle2}
\end{figure}
The predictive mean is still overly wiggly, but it does appear to be
reasonably
smooth also reveals structure in the
...
make sense considering the output is measuring a whiplash effect.  However,
...
\citet{kennedy:ohagan:2001} were the first to propose a statistical framework
for combining simulation output and noisy field observations for model
calibration. Their hierarchical model links field measurements to potentially
biased computer simulations.  It is paired with a Bayesian inferential framework for
jointly estimating, using data from both processes, the bias, noise level, and
any parameters required to run the computer simulation---so-called {\em
calibration parameter(s)}---but which cannot be controlled or observed in the
field.  The setup, which we review below, has many attractive features, however
it scales poorly when simulations get large.  We explain how
\citet{gramacy:bingham:etal:2014} modified that setup using \pkg{laGP} and
...
physical or field experiment $F$ which follows a design that respects
...
following coupling of $M$ and $F$.  Let $y_j^F(x)$ denote the $j^\mathrm{th}$
replication of the field ``run'' at $x$, and $y^M(x,u)$ denote the (deterministic)
...
$u^*$, plus a bias term acknowledging that there can be systematic
...
\begin{align}
&& y^F_j(x) &= y^R(x) + \varepsilon_{xj},& \varepsilon_{xj} 
&\iidsim \mN(0, \sigma_\varepsilon^2),  \;\;\; j=1, \dots, n_x. \nonumber \\
\mbox{giving } && y^F_j(x) &= y^M(x, u^*) + b(x) + \varepsilon_{xj} \label{eq:kohmodel},
\end{align}
The unknown parameters are $u^*$, $\sigma_\varepsilon^2$, and the discrepency
or bias $b(\cdot)$.

If evaluating the computer model is fast, then inference can proceed via
residuals $y^F_j(x) - y^M(x, u)$, which can be computed at will for any
...
locations $(x, u)$.  KOH recommend a GP prior for $y^M$, however rather learn
$\hat{y}^M$ in isolation, using just the $N_M$ runs, as we
have been doing throughout this document, they recommend inference joint with
...
\citet{gramacy:bingham:etal:2014} proposed a thriftier approach pairing
...
fancy, but its really just back-to-basics: fitting the emulator
$\hat{y}^M(\cdot, \cdot)$ separately or independently from the bias, using
only the outputs of runs at a design of $N_M$ inputs $(x, u)$.
\citeauthor{liu:bayarri:berger:2009}'s justification for modularization
...
\citeauthor{gramacy:bingham:etal:2014} motivate modularization from
...
\citeauthor{gramacy:bingham:etal:2014} recommend
viewing calibration as an optimization which acts as the glue that ``sticks it
all together''.
\begin{algorithm}[ht!]
\begin{algorithmic}[1]
...
\STATE $\hat{\theta} \leftarrow \mbox{\tt mleGP}(D^M_{N_M}[I])$
        \hfill \COMMENT{local MLE of correlation parameter(s)}
\STATE $y^{M|u}_j \leftarrow \mbox{\tt muGP}(x^F_j \mid D^M_{N_M}[I], 
        \hat{\theta})$
        \hfill \COMMENT{predictive mean emulation 
        following Eq.~(\ref{eq:preds2})}
\ENDFOR
\STATE $\hat{b}_{N_F} \leftarrow  Y^F_{N_F} - Y^{M|u}$
        \hfill \COMMENT{vectorized bias calculation}
\STATE $D_{N_F}^{\hat{b}} \leftarrow (\hat{b}_{N_F}, X^F_{N_F})$
        \hfill \COMMENT{create data for estimating $\hat{b}(\cdot)|\mu$}
\STATE $\hat{\theta} \leftarrow \mbox{\tt mleGP}(D_{N_F}^{\hat{b}})$
        \hfill \COMMENT{full GP estimate of $\hat{b}|u$}
\RETURN $\mbox{\tt predGP}(Y^F_{N_M} | X_{N_M}, \hat{\theta})$ 
...
\label{alg:obj}
\end{algorithm}
Algorithm \ref{alg:obj} provides pseudocode comprised library
...
$N_F$ field data inputs, paired with a $N_F$ vector of $y^F$ observations
...
discrepancies as $\hat{b}_{N_F} = Y^F_{N_F} -
...
regression model trained on data $D^{\hat{b}}_{N_F} = (\hat{b}_{N_F},
...
\citeauthor{gramacy:bingham:etal:2014} suggest a derivative-free 
approach via the mesh adaptive direct search (MADS) algorithm \citep{AuDe2006}
known as \pkg{NOMAD} \citep{Le09b}. The authors of the \pkg{crs} package
\citep{crs} provide \code{snomadr}, an \proglang{R} wrapper to the underlying
\proglang{C++} \pkg{NOMAD} implementation. MADS/\pkg{NOMAD} proceeds by
successive pairs of {\em search} and {\em poll} steps, trying inputs to the
objective function on a sequence of meshes which are refined in such a way as
to guarantee convergence to a local optima under very weak regularity
conditions; for more details see \cite{AuDe2006}.

As MADS is a local solver, \pkg{NOMAD} requires initialization.  
\citeauthor{gramacy:bingham:etal:2014} recommend choosing starting
...
$D^{\hat{b}}_{N_F}$ and $\hat{\theta}$ from the optimization, or quickly
re-computing them via \code{discrep.est}, it builds a predictor for the field
at new $x$ locations. Emulations and biases are thus combined into
distribution for the $y^F(x)|u^*$, a sum of Student-$t$ random variables for
...
function to simulate real data under a process like (\ref{eq:kohmodel}).
<<>>=
bias <- function(x) 
  {
    x<-as.matrix(x)   
...
augmented with bias and noise processes as follows.
<<cache=TRUE>>=
...
@
The code uses \code{Y} denote field data observations $Y_{N_F}^F$ with
$N_M =$~\code{ny}~$=$~\Sexpr{ny}, which stores two replicates at
each $X^F_{N_F} =$ \code{X} location.  \citet{gramacy:bingham:etal:2014}
illustrated this example with ten replicates.  We keep it smaller here
for faster execution in live demonstration.
...
The following block sets priors and specifies details of
the model(s) to be estimated.
<<>>=
bias.est <- TRUE
methods <- rep("alc", 2)
da <- d <- darg(NULL, XU)
...
the and the bias parameters respectively.  The prior is completed with
a (log) prior density on the calibration parameter, $u$, which we choose
to be independent Beta with a mode in the middle of the space.
<<>>=
...
Now we are ready to evaluate the objective function on a ``grid'' 
to search for a good initialization for a more penetrating search by 
\pkg{NOMAD}.  The following code builds the ``grid'' via a space-filling
...
@
By default, \code{fcalib} echoes the input and calculated objective value
(log likelihood or posterior probability) to the screen.  This can be useful
for tracking progress for an optimization, say via \pkg{NOMAD}, however
we suppress this prevent clutter.  The \code{fcalib} function
has an argument called \code{save.global} which (when not \code{FALSE}) 
causes the information
which would be printed to the screen to be saved in a global variable called 
\code{fcalib.save} in the environment indicated (e.g., \code{save.global = .GlobalEnv}), 
which can be useful for visualization once the optimization
...
The next code chunk loads the \pkg{crs} library which contains \code{snomadr},
...
of \code{u}-values and onces placed along stencils in search of the
\code{u}-value which maximizes the likelihood (or posterior probability).
...
\label{f:usurf}
\end{figure}
Figure \ref{f:usurf} shows an image plot of the surface, with lighter-
colored values indicating a larger value of likelihood/posterior probability.
The initialization points (open circles), evaluations along the \pkg{NOMAD}
search (black dots), and the ultimate value found in optimization (green dot)
are also shown.
...
\citet{gramacy:bingham:etal:2014} offer an explanation.  The KOH
...
predictors \citep{bah:joseph:2012} and the introduction of auxiliary inputs
...
non-stationary features in response surface models, particularly for computer
...
The \pkg{laGP} package is under active development, and the corpus of code
was developed with ease of extension in mind.  The calibration application from
Section \ref{sec:calib} is a perfect example: simple functions
tap into local GP emulators and full GP discrepancies alike,
and are paired with existing direct optimizing subroutines from other packages
for a powerful solution to large scale calibration problems that are becoming
commonplace in the recent literature.  

The library comprises of roughly fifty \proglang{R} functions, although barely
...
\code{aGP.R}) which implement similar (usually with identical output, our a
superset of output) subroutines entirely in \proglang{R}. These were 
used as stepping stones in the development of the \proglang{C} versions,
however they remain as a window into the inner-workings of the package and as
a skeleton for curious users development of new extensions.  The local
...
As one example we provide a final illustration, here, of how some of the basic
functionality in the package---only utilizing full (non local) GP
subroutines---was useful in solving hard blackbox optimization problems under
constraints.
\citet{gramacy:etal:2014} showed how the augmented Lagrangian, an apparatus
...
particular was linear), but where the constraints required (potentially expensive)
simulation.  Searching for an optimal valid setting of the inputs to the blackbox 
function could be substantially complicated by a difficult-to-map
constraint satisfaction boundary.
...
@
The input space is two-dimensional, and the goal is to find a solution \code{xstar}
in the bounding box, \code{B}
<<cache=TRUE>>=
B <- matrix(c(rep(0,2),rep(1,2)),ncol=2)
B
@
for which \verb!all(blackbox(xstar)$c <= 0!, meeting the constraint, yet
\verb!blackbox(xstar)$obj! minimized.  There are three local minima which satisfy
the constraints; for more details see \citet{gramacy:etal:2014}.

The \pkg{laGP} package implements a hybrid EI--augmented Lagrangian method, which is described in detail by 
the above reference, in the function \code{optim.auglag}.
<<cache=TRUE>>=
out.aug <- optim.auglag(blackbox, B, ab=c(3/2,8), end=100, verb=0)
@
The output object contains information about the inputs that were sent to
the blackbox for evaluation, the corresponding outputs obtained in terms of objective and
constraints, and also tracks the best value of the objective with a valid input 
(i.e., the progress).
<<>>=
out.aug$prog[100]
...
(out.aug$X[valid,])[m,]
@
The algorithm is initialized stochastically, and in 99\% of restarts it finds
the correct global (valid) minimum (0.6) by the end of the budget of 100
blackbox evaluations.  Inspecting the code in \code{optim.auglag} reveals
several useful GP-related functions provided in the package, such as
\code{updateGP},
\code{rbetter}, \code{alGP}.

By way of comparison, consider deploying the method of simulated annealing
\citep[SA;][]{kirk:1983} as implemented by \code{method="SANN"} in the
\code{optim} function for \proglang{R}.  The \code{optim} function only supports
box constraints, so we cannot provide the \code{blackbox} function directly.
To convert a mixed objective and constraint function into a workable hybrid objective,
we deploy the additive penalty method (APM).
<<cache=TRUE>>=
...
out.sann
@
Note that the final value typically does not satisfy the constraints as
the search typically approaches local optima from the invalid side
of the boundary.  If the final solution isn't valid, it is usually very
close to the boundary.
<<>>=
blackbox(out.sann$par)
@
The objective value found is near 0.6 only about 60\% of the time in
repeated random restarts despite performing hundreds of blackbox evaluations.
...
established via the \pkg{parallel} or \pkg{snow} packages.
...
on multi-core machines.  The most important one is \code{aGP} which allows
...
the time of writing is not the default in any of the standard builds----then
...
machine is to provide an \code{omp.threads} argument, say to \code{aGP}, which
is bigger than one.  If \code{OpenMP} support is not enabled then you will get
a warning.  If you are working within a well-managed supercomputing facility,
then it may be that \proglang{R} has already been compiled with \pkg{OpenMP}
support. If not, perhaps it is worth requesting that it be re-compiled as
there are many benefits to doing so, beyond those which extend to the
\pkg{laGP} package.  For example, many linear algebra intensive packages,
of which \pkg{laGP} is one, benefit from liking to MKL libraries from Intel,
in addition to \pkg{OpenMP}.
...
becoming the standard setup for desktops and laptops alike. For those with
experience compiling \proglang{R} packages from source, the procedure is quite
straightforward and does not require compiling or installing a bespoke version
of
\proglang{R}.  Obtain the package source (e.g., from CRAN) and, before compiling,
open up the package and make two small edits to laGP/src/Makevars.  These instructions
assume a GCC compiler.  For other compilers, please consult documentation for appropriate
flags.
...
either doing ``R CMD INSTALL'' on the modified directory our, or after re-tarring
...
be highly archictecture and operating system specific, therefore the very
basic instructures here may not work widely.  They have been tested on a
varity of Unix-alikes including Intel-based Ubuntu Linux and OSX systems. 

First compule the \code{alc_gpu.cu} file into an object using the Nvidia 
\proglang{CUDA} compler.  E.g., after untarring the packge change into \code{laGP/src}
and do
\begin{verbatim}
...
augmenting changes made above to accommodate \pkg{OpenMP} support, as
described above.  \pkg{OpenMP} (i.e., using multiple CPU threads) brings out
the best in our GPU implementation.

\begin{enumerate}
\item Add \verb!-D_GPU! to the \verb!PKG_FLAGS!
\item Add \verb!alc_gpu.o -L /software/cuda-5.0-el6-x86_64/lib64 -lcudart! to the
...
usual.  Alternatively, use \verb!make allgpu! to via the definitions in the
...
you wish to use, and indicating more you actually have will trip an
error.  The latter, which defaults to \code{gpu.threads = num.gpus}, specifies
how many CPU threads should be used to queue GPU jobs.  Having \code{gpu.threads < num.gpus}
is an inefficient use of resources, whereas \code{gpu.threads > num.gpus}, up to 
...
be included in the package to have more than one \code{gpu.thread}.
...
substantial speedups.  \citet{gramacy:niemi:weiss:2014} observe up to 50\%
