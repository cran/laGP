...
print(nth)
@
If your machine has fewer cores, is not compiled with \code{OpenMP} or caps the
number of \code{OpenMP} threads to a lower value (see Appendix \ref{sec:omp}),
...
the time of writing standard in most builds----then
...
a \code{omp.threads} value which is greater than the max allowed by the
